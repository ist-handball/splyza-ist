...
window.addEventListener("error", function(e) {
    // filenameが取得できない場合（CORS制限のある外部スクリプトなど）の対応
    const filename = e.filename ? e.filename.split('/').pop() : 'unknown';
    const errorMsg = `JS Error: ${e.message}\nFile: ${filename}\nLine: ${e.lineno}:${e.colno}`;
    console.error(errorMsg);
    
    // 自作スクリプト (app.js) 起因のエラーのみ alert を表示
    const isLocalScript = e.filename && (e.filename.indexOf("app.js") !== -1 || e.filename.indexOf(window.location.host) !== -1);
    if (isLocalScript) {
...
async function initFirebase(config) {
    ...
    try {
        ...
        // 大学や社内LAN等の制限ネットワーク(gRPC/WebSocketsブロック)対策としてロングポーリングを強制
        db.settings({
